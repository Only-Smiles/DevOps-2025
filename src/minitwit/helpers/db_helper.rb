module DbHelper
  # Database connection with Sequel
  def self.db
    @db ||= Sequel.sqlite(MiniTwit::DATABASE)
  end

  # Instance method to use the connection
  def db
    DbHelper.db
  end
  ...
  def add_message(user_id, text)
    db[:message].insert(author_id: user_id, text: text, pub_date: Time.now.to_i, flagged: 0)
  end
  ...
  def get_followers(user_id, limit = 100)
    db[:user]
      ...
      .where(Sequel.qualify(:follower, :who_id) => user_id)
      .limit(limit)
      .select(:username)
      .map(:username)
  end
